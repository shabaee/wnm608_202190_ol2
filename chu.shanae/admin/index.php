...
<form method="post" action="{$_SERVER['PHP_SELF']}?id=$id&action=$createorupdate">
    ...
    <div class="form-control">
        ...
        <input class="form-input" name="product-name" id="product-name" type="text" placeholder="Enter Product Name" value="$o->name">
    </div>
    <div class="form-control">
        <label class="form-label" for="product-price">Price</label>
        <input class="form-input" name="product-price" id="product-price" type="number" placeholder="Enter Product Price" value="$o->price" min="1" max="500" step="0.01">
    </div>
    <div class="form-control">
    ...
    <div class="form-control">
        ...
        <input class="form-input" name="product-images" id="product-images" type="text" placeholder="Enter Product Images Comma Separated" value="$o->images">
    </div>
    <div class="form-control">
...
try {
    ...
    if (isset($_GET['action'])) {
        switch ($_GET['action']) {
            case "update":
                $statement = $conn->prepare("UPDATE `products` SET `name`=?, `price`=?, `quantity`=?, `category`=?, `description`=?, `thumbnail`=?, `images`=?, `date_modify`=NOW() WHERE `id`=?");
                $statement->execute([
                    $_POST['product-name'],
                    $_POST['product-price'],
                ...
                break;
            case "create":
                $statement = $conn->prepare("INSERT INTO `products` (`name`, `price`, `quantity`, `category`, `description`, `thumbnail`, `images`, `date_create`, `date_modify`) VALUES (?,?,?,?,?,?,?,NOW(),NOW())");
                $statement->execute([
                    $_POST['product-name'],
                    $_POST['product-price'],
                    $_POST['product-quantity'],
...
<body>
    <header class="navbar">
        <div class="container display-flex">
            <div class="flex-none">
                <div class="headings-white">Product Admin</div>
            </div>
